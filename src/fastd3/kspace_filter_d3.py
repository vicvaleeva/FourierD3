...
class KSpaceFilterCN(KSpaceFilter):
    ...
    def forward(self, mesh_values: torch.Tensor) -> torch.Tensor:
        ...
        # mesh_values has (n_species, nx, ny, nz) dimensions
        ...
        mesh_hat = torch.fft.rfftn(mesh_values, norm=self._fft_norm, dim=(1, 2, 3)) # (n_species, nx, ny, nz)
        struc = mesh_hat.flatten(1) # (n_species, n_k)
        interm = torch.einsum('kab,bk->ak', self._kfilter, struc)# (n_species, nk)
        phi_mesh = torch.fft.irfftn(interm.view(mesh_hat.shape), norm=self._ifft_norm, s=mesh_values.shape[-3:], dim=(1, 2, 3)) #(n_species, nx, ny, nz)
